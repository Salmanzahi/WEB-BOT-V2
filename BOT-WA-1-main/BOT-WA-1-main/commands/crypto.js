...
module.exports = async (command, args, msg, user, db) => {
      ...
      Object.keys(db.users).forEach((userId) => {
        ...
        if (u.debt > 0) {
          ...
          if (totalCollateral === 0 || u.debt > totalCollateral * 0.85) {
            console.log(
              `💀 LIQUIDATION: User ${u.name || userId} bangkrut karena harga pasar turun.`,
            );
  ...
  if (command === "buycrypto") {
    ...
    if (!COIN_IDS[koin])
      return msg.reply(
        `❌ Koin tidak ada. List: ${Object.keys(COIN_IDS).join(", ")}`,
      );
    if (!market.prices[koin])
      return msg.reply("❌ Data harga sedang loading... coba lagi.");
    if (isNaN(nominal) || nominal < 10000)
      return msg.reply("❌ Min beli Rp 10.000");
    if (user.balance < nominal)
      return msg.reply(`❌ Uang kurang. Saldo: Rp ${fmt(user.balance)}`);
  ...
  if (command === "sellcrypto") {
    ...
    if (!user.crypto[koin] || user.crypto[koin] <= 0)
      return msg.reply("❌ Dompet kosong.");
  ...
  if (command === "margin") {
    ...
    if (isNaN(nominal) || nominal < 100000)
      return msg.reply("❌ Min margin Rp 100.000");

    // Limit Margin: 3x Saldo
    const maxLoan = user.balance * 3;
    if (user.debt + nominal > maxLoan)
      return msg.reply(
        `❌ Limit Margin Mentok! Sisa limit: Rp ${fmt(maxLoan - user.debt)}`,
      );
  ...
  if (command === "paydebt") {
    ...
    saveDB(db);
    return msg.reply(
      `✅ Hutang lunas Rp ${fmt(bayar)}. Sisa hutang: Rp ${fmt(user.debt)}`,
    );
  }
